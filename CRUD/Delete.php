...
if (isset($_GET["deleteTransactionID"])) {
    $data_to_be_deleted = $_GET["deleteTransactionID"];
    $result = deleteTransactions($data_to_be_deleted);
    if ($result) {
        echo "<h1>Delete Transaction Data with ID $data_to_be_deleted SUCCESS</h1>";
    } else {
        echo "<h1>Delete Transaction Data with ID $data_to_be_deleted FAILED</h1>";
    }
}
